...
def fetch_gsc_monthly_by_page(service, site_url, start_dt, end_dt, country_iso3=None, section_path=None):
    ...
    for m_start, m_end in month_range(start_dt, end_dt):
        body = {
            "startDate": str(m_start), "endDate": str(m_end),
            "dimensions": ["page"], "type": "web", "aggregationType": "auto",
        }
        filters = []
        if country_iso3:
        ...
        if rows:
            frames.append(pd.DataFrame([
                {"page": r["keys"][0], "month": pd.to_datetime(m_start),
...
def fetch_gsc_daily_evergreen(service, site_url, start_dt, end_dt, country_iso3=None, section_path=None, page_size=25000):
    ...
    body = {
        ...
        "dimensions": ["page","date"], "rowLimit": page_size, "type": "web", "aggregationType": "auto",
    }
    filters = []
    if country_iso3:
        filters.append({"dimension": "country", "operator": "equals", "expression": country_iso3})
    ...
    while True:
        ...
        for r in rows:
            rows_all.append({
                "page": r["keys"][0], "date": pd.to_datetime(r["keys"][1]),
                "clicks": r.get("clicks", 0), "impressions": r.get("impressions", 0),
