...
- No Service Account. Only user OAuth.
- Reads nested secrets: [gcp_oauth_client] -> [gcp_oauth_client.web]
- If secrets are missing, lets you paste client_id, client_secret, redirect_uri.
- After login, lists GSC properties to verify access.
...
# -------------------------------------------------------------
# Config loader (secrets first, UI fallback)
# -------------------------------------------------------------
...
".join(sites))
    ...
    # Intentar manejar el callback si ya volvió con ?code=
    handle_callback()
import streamlit as st
from google_auth_oauthlib.flow import Flow
from googleapiclient.discovery import build
